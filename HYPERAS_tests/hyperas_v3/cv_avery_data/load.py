...
def data():
    ...
    scaled_X = scaler.fit_transform(X)
    scaled_y = scaler.fit_transform(y.reshape(-1,1))
    X_train, X_fulltest, y_train, y_fulltest = train_test_split(scaled_X, scaled_y, test_size = 0.8, random_state=42)
...
history = best_model.fit(x=X_train,y=y_train,epochs=2000,validation_data=valid_set,batch_size=15,verbose=2)
