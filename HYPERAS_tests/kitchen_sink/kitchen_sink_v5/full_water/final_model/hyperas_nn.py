...
# Standard scaling, 3 Fold CV, up to 5 layers, standard Adam optimizer default settings, 500 epochs. 
...
def create_model(X,y):
    ...
    for train, test in kfold.split(sX, sy):
        model = Sequential()
        model.add(Dense(100, input_shape=in_dim))
        ...
        # rather than fix epochs, let it go until the model stops improving. 
        # In scaled data, it is unclear what loss corresponds to 1 wavenumber (4.5e-6 hartrees) in scaled data. emprical guess of 1.0e-7.
        callbacks = [EarlyStopping(monitor='loss', min_delta=5.0e-9, patience=5000)]
        model.compile(loss='mse', optimizer=opt, metrics=['mae'])
        model.fit(x=sX,y=sy,epochs=50000,batch_size=10,verbose=1, callbacks=callbacks)
        ...
        predicted_scaled_y = model.predict(sX)
        predicted_y = scaler.inverse_transform(predicted_scaled_y.reshape(-1,1))
        actual_y = scaler.inverse_transform(sy)
        mae = mean_absolute_error(actual_y, predicted_y)
        loss = mean_squared_error(actual_y, predicted_y)
        fcv_losses.append(loss)
        fcv_maes.append(mae)
    print("Test MSEs: ", cv_losses)
    print("Test MAEs: ", cv_maes)
    print("Full MSEs: ", fcv_losses)
    print("Full MAEs: ", fcv_maes)
